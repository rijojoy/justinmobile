    ...
    Cron::add('reminder1', '* * * * *', function() {
                    ...
                    foreach( $orders as $order)
                    {
                        ...
                        if($order->daysold == 0)
                        {
                            ...
                            if($orderNote1 == null)
                             {
                                    $person = json_decode($order[0]->personal_info, true);
                            ...
                            $mail_stat =  \Mail::send('emails.orders.updated', array('body' => $tpl1), function($message) use ($order) {
                        ...
                        if($order->daysold == 7)
                        {
                            ...
                            if($orderNote1 == null)
                             {
                                    $person = json_decode($order[0]->personal_info, true);
                                    $tpl2 = str_replace('%%name%%',$person['name'],$selectTpl7[0]->body);
                                        $note = new \OrderNote;
                                        $note->order_id = $order->id;
                                        $note->status_id = 12;
                                        $note->user_id = 9;
                                        ...
                                        $order->save();
                                     /* Its time to send email */
                            $mail_stat =  \Mail::send('emails.orders.updated', array('body' => $tpl2), function($message) use ($order) {
                            $message->from(\Config::get('mail.from.address'), \Config::get('mail.from.name'));
                            $message->to($person['email'], $person['name'])->subject('Sell Order #' . $order->id . ' Update');
                        });
                            /* End of its time to send email */   
                        ...
                        if($order->daysold == 14)
                        {
                            ...
                            if($orderNote1 == null)
                             {
                                    $person = json_decode($order[0]->personal_info, true);
                                    $tpl3 = str_replace('%%name%%',$person['name'],$selectTpl14[0]->body);
                                        $note = new \OrderNote;
                                        $note->order_id = $order->id;
                                        $note->status_id = 15;
                                        $note->user_id = 9;
                                        ...
                                        $order->save();
                                     /* Its time to send email */
                            $mail_stat =  \Mail::send('emails.orders.updated', array('body' => $tpl3), function($message) use ($order) {
                            $message->from(\Config::get('mail.from.address'), \Config::get('mail.from.name'));
                            $message->to($person['email'], $person['name'])->subject('Sell Order #' . $order->id . ' Update');
                        });
                            /* End of its time to send email */   
