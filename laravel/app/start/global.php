...
Event::listen('cron.collectJobs', function() {
    Cron::add('reminder1', '* * * * *', function() {
                    ...
                    $orders = \Order::select(DB::Raw("DATEDIFF(now(),`created_at`) as 'daysold'"),'id','product_id','personal_info','status_id')->OrderBy(DB::Raw("DATEDIFF(now(),`created_at`)"),'DESC')->get();
                    /* Select Template */
                    ...
                    foreach( $orders as $order)
                    {
                       ...
                       if($order->status_id == 2 || $order->status_id == 12 || $order->status_id == 15)
                       {
                            if($order->daysold == 1)
                            {
